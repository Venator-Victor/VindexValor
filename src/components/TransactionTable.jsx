              ...
              {sortedTransactions.map(tx => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-medium text-gray-900 dark:text-gray-50" title={tx.description || tx.name}>
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="truncate">{tx.description || tx.name}</span>
                        {tx.is_recurring && <History size={16} className="text-primary shrink-0" title={t('transactions.recurring_transaction_title')} />}
                        {tx.type === 'payment' && <span className="text-[10px] bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300 px-2 py-0.5 rounded-full whitespace-nowrap shrink-0">{t('transactions.type_payment')}</span>}
                      </div>
                    </td>
