...
const DeleteConfirmationDialog = ({ open, onOpenChange, title, description, onConfirm, isLoading = false }) => {
  ...
  return (
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="bg-white dark:bg-vindex-card text-gray-900 dark:text-gray-100 border-gray-200 dark:border-vindex-border rounded-xl">
        <AlertDialogHeader>
          <AlertDialogTitle>{title || t('common.delete_confirm_title')}</AlertDialogTitle>
        ...
        <AlertDialogFooter className="flex-row gap-2 sm:justify-stretch">
          ...
          >
            {isLoading ? t('common.deleting') : t('common.delete_confirm_action')}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
